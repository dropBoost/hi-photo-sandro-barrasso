...
import PreventivoPdfDocument from "@/components/pdfPreventivo/MyPdfPreventive";
...
export async function GET(req) {
  try {
    ...
    const stream = await renderToStream(
      <PreventivoPdfDocument data={payload} />
    );

    return new Response(stream, {
      headers: {
        "Content-Type": "application/pdf",
        "Content-Disposition": `inline; filename="preventivo-${idPreventivo}.pdf"`,
      },
    });
  } catch (error) {
